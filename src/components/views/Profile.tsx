    ...
    <div className="max-w-4xl mx-auto space-y-10 pb-20">
        ...
        <div className="px-10 -mt-24 flex flex-col md:flex-row items-end gap-10 relative z-10 w-full">
          <div className="relative group">
            ...
            <img 
              ...
              className="w-44 h-44 rounded-[3rem] bg-white dark:bg-zinc-900 border-[10px] border-white dark:border-zinc-900 shadow-2xl object-cover transform hover:scale-105 transition-transform" 
              alt="Avatar" 
            />
            {isEditing && (
          ...
          <div className="pb-8 pt-6 space-y-3 flex-1 w-full text-center md:text-left">
            ...
            <div className="flex flex-wrap items-center gap-6 justify-center md:justify-start">
              ...
              {profile?.isVerified && (
                <p className="text-green-500 font-bold text-[10px] uppercase tracking-widest flex items-center gap-2 bg-green-50 dark:bg-green-500/10 px-3 py-1 rounded-full border border-green-100 dark:border-green-500/20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-8 space-y-8">
          ...
          {!profile?.familyId && (
            <div className="bg-black dark:bg-zinc-950 text-white p-12 rounded-[3.5rem] shadow-2xl space-y-10 relative overflow-hidden dark:border dark:border-white/10">
                    ...
                    <div className="bg-white/5 border border-white/10 p-8 rounded-[3rem] space-y-6 flex flex-col hover:bg-white/[0.08] transition-all">
                      ...
                      <div className="space-y-4">
                        <input 
                          ...
                          className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm font-medium outline-none focus:bg-white/10 transition-all placeholder:text-white/20 text-white"
                        />
                        <button 
                    ...
                    <div className="bg-white/5 border border-white/10 p-8 rounded-[3rem] space-y-6 flex flex-col hover:bg-white/[0.08] transition-all">
                      ...
                      <div className="space-y-4">
                        <input 
                          ...
                          className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm font-bold outline-none focus:bg-white/10 transition-all uppercase tracking-[0.4em] text-center placeholder:text-white/20 text-white"
                        />
                        <button 
                          onClick={joinFamily}
          ...
          <div className="bg-gray-50 dark:bg-zinc-900/50 p-12 rounded-[4rem] border border-black/5 dark:border-white/5 space-y-8">
             <div className="flex items-center justify-between">
                <div>
                   <h3 className="text-2xl font-bold italic serif tracking-tight text-black dark:text-white">Identity & Trust</h3>
                   <p className="text-xs text-gray-400 dark:text-gray-500 font-medium mt-1">Verify age and identity to unlock financial and authority features.</p>
                </div>
                <div className={cn(
                  "p-4 rounded-3xl",
                  profile?.isVerified ? "bg-green-500/10 text-green-600 dark:text-green-400" : "bg-orange-500/10 text-orange-600 dark:text-orange-400"
                )}>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2.5rem] border border-black/[0.03] dark:border-white/[0.03] flex flex-col justify-between">
                   <div className="space-y-2">
                     <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-gray-500">Age Verification</p>
                     <h4 className="text-3xl font-black italic serif text-black dark:text-white">{profile?.age || '--'} <span className="text-xs not-italic font-bold text-gray-300 dark:text-gray-600 uppercase ml-2">Years Old</span></h4>
                   </div>
                   {!profile?.isVerified && (
                     <button 
                       onClick={() => updateDoc(doc(db, 'users', user.uid), { isVerified: true, age: 24 })}
                       className="w-full bg-black dark:bg-white text-white dark:text-black py-4 rounded-2xl font-bold text-xs uppercase tracking-widest hover:scale-[1.02] active:scale-95 transition-all"
                      >
          ...
          {user.email === 'cyrongau@gmail.com' && (
            <div className="bg-red-50 dark:bg-red-500/10 p-6 rounded-[2.5rem] border border-red-100 dark:border-red-500/30 flex items-center justify-between">
               <div>
                  <h4 className="font-bold text-red-900 dark:text-red-200 border-b border-red-200 dark:border-red-500/30 pb-1 mb-1">Developer Override</h4>
                  <p className="text-xs text-red-700 dark:text-red-400 font-medium">Toggle System Administrator Settings</p>
               </div>
               <button
                  onClick={() => updateDoc(doc(db, 'users', user.uid), { isSystemAdmin: !profile?.isSystemAdmin })}
                  className={cn("px-6 py-3 rounded-[1rem] font-bold text-xs uppercase tracking-widest", profile?.isSystemAdmin ? "bg-red-600 text-white" : "bg-white dark:bg-zinc-800 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-500/30")}
               >
          ...
          <div className="bg-white dark:bg-zinc-900 p-12 rounded-[3.5rem] border border-black/5 dark:border-white/5 shadow-sm space-y-12">
            <div className="flex items-center justify-between border-b border-gray-50 dark:border-zinc-800 pb-8">
              <h3 className="text-2xl font-bold italic serif tracking-tight text-black dark:text-white">Profile Integrity</h3>
              <div className="flex items-center gap-2">
                <span className={cn(
                  "px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest",
                  isEditing ? "bg-orange-100 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400" : "bg-green-100 dark:bg-green-500/20 text-green-600 dark:text-green-400"
                )}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-3">
                ...
                <input 
                  ...
                  className="w-full bg-gray-50 dark:bg-zinc-800/50 border border-black/5 dark:border-white/5 text-black dark:text-white rounded-2xl p-5 font-bold focus:ring-1 focus:ring-black/10 dark:focus:ring-white/10 outline-none disabled:opacity-50" 
                />
              </div>
              ...
              <div className="space-y-3">
                ...
                <select
                  ...
                  className="w-full bg-gray-50 dark:bg-zinc-800/50 border border-black/5 dark:border-white/5 text-black dark:text-white rounded-2xl p-5 font-bold focus:ring-1 focus:ring-black/10 dark:focus:ring-white/10 outline-none disabled:opacity-50 appearance-none"
                >
              ...
              <div className="md:col-span-2 space-y-3">
                ...
                <textarea 
                  ...
                  className="w-full bg-gray-50 dark:bg-zinc-800/50 border border-black/5 dark:border-white/5 text-black dark:text-white rounded-2xl p-5 font-bold min-h-[120px] focus:ring-1 focus:ring-black/10 dark:focus:ring-white/10 outline-none disabled:opacity-50 leading-relaxed" 
                />
              </div>
            </div>
            ...
            {isEditing && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="pt-8">
                 <button 
                  ...
                  className="w-full bg-black dark:bg-white text-white dark:text-black py-6 rounded-3xl font-bold text-xl shadow-2xl shadow-black/30 dark:shadow-white/10 flex items-center justify-center gap-4 hover:scale-[1.01] active:scale-[0.98] transition-all"
                 >
          ...
          {profile?.familyId && (
            <div className="bg-orange-50 dark:bg-orange-950/20 border border-orange-100 dark:border-orange-900/30 p-12 rounded-[4rem] space-y-8 relative overflow-hidden group">
               <div className="absolute top-0 right-0 p-10 text-orange-200/20 dark:text-orange-500/10 group-hover:scale-110 group-hover:rotate-6 transition-transform duration-1000">
               ...
               <div className="relative z-10 flex flex-col md:flex-row items-center md:items-end justify-between gap-6">
                  <div className="flex flex-col md:flex-row items-center gap-6">
                     <div className="w-24 h-24 rounded-[2rem] bg-orange-100 dark:bg-orange-900/50 border border-orange-200 dark:border-orange-500/20 overflow-hidden relative group/photo shrink-0 shadow-inner">
                        <img 
                           src={family?.photoURL || `https://picsum.photos/seed/family-${profile.familyId}/200/200`} 
                           className="w-full h-full object-cover" 
                           alt="Family" 
                           referrerPolicy="no-referrer"
                        />
                        {user.uid === family?.ownerId && (
                           <label className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center opacity-0 group-hover/photo:opacity-100 transition-all cursor-pointer backdrop-blur-sm">
                     ...
                     <div className="text-center md:text-left space-y-3">
                       <span className="text-[10px] font-black text-orange-400 dark:text-orange-500 uppercase tracking-[0.3em] block">Operational Home</span>
                       <h3 className="text-4xl font-black text-orange-950 dark:text-orange-100 italic serif mt-2 tracking-tight">{family?.name}</h3>
                       {user.uid === family?.ownerId && (
                         <button 
                          onClick={() => setShowFamilyBannerInput(!showFamilyBannerInput)}
                          className="bg-white/50 dark:bg-black/50 backdrop-blur-md text-orange-900 dark:text-orange-200 border border-orange-200/50 dark:border-orange-500/20 px-4 py-2 rounded-xl shadow-sm hover:bg-white/80 dark:hover:bg-black/80 transition-all flex items-center gap-2 mx-auto md:mx-0"
                         >
                  ...
                  <div className="flex flex-col items-center md:items-end gap-2">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-black text-orange-400 dark:text-orange-500 uppercase tracking-[0.2em]">Group invite Code</span>
                      {user.uid === family?.ownerId && (
                         <button onClick={regenerateInviteCode} title="Generate New Code" className="text-orange-400 hover:text-orange-600 dark:hover:text-orange-300 transition-colors">
                    ...
                    <button 
                      ...
                      className="bg-white dark:bg-zinc-900 border border-orange-200 dark:border-orange-900/50 px-6 py-3 rounded-2xl flex items-center gap-4 hover:bg-orange-100 dark:hover:bg-orange-900/70 transition-all font-mono font-black text-orange-900 dark:text-orange-300 shadow-sm active:scale-95"
                    >
                      {family?.inviteCode}
               ...
               <AnimatePresence>
                 {showFamilyBannerInput && (
                   <motion.div 
                     ...
                     className="relative z-10 bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-orange-100 dark:border-orange-900/30 shadow-sm mt-4 flex flex-col md:flex-row items-center gap-4"
                   >
                     <input 
                        type="url" 
                        ...
                        className="flex-1 bg-orange-50 dark:bg-zinc-800 border border-orange-100 dark:border-orange-900/30 text-black dark:text-white rounded-xl p-3 text-sm outline-none focus:ring-1 focus:ring-orange-300 w-full md:w-auto"
                     />
               ...
               {user.uid === family?.ownerId && joinRequests.length > 0 && (
                 <div className="relative z-10 bg-white dark:bg-zinc-900 rounded-3xl p-6 border border-orange-100 dark:border-orange-900/30 shadow-sm">
                   <h4 className="text-sm font-bold text-orange-950 dark:text-orange-100 mb-4 flex items-center gap-2">Pending Join Requests <span className="bg-orange-100 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400 px-2 py-0.5 rounded-full text-[10px]">{joinRequests.length}</span></h4>
                   <div className="space-y-3">
                     {joinRequests.map(req => (
                       <div key={req.id} className="flex flex-col md:flex-row items-center justify-between gap-4 p-4 bg-orange-50 dark:bg-orange-950/20 rounded-2xl border border-orange-100/50 dark:border-orange-900/30">
                         <div className="flex-1">
                           <p className="font-bold text-sm text-orange-950 dark:text-orange-100">{req.userName}</p>
                           <p className="text-xs text-orange-600/70 dark:text-orange-400/70">{req.userEmail}</p>
                         </div>
                         <div className="flex gap-2 w-full md:w-auto">
                           ...
                           <button onClick={() => handleDenyJoin(req)} className="flex-1 md:flex-none bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400 px-4 py-2 rounded-xl text-xs font-bold hover:bg-red-200 dark:hover:bg-red-500/30 active:scale-95 transition-all">Deny</button>
                         </div>
                       </div>
                     ))}
                   </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 relative z-10 pt-4">
                  {familyMembers.map(member => (
                    <motion.div 
                      key={member.uid} 
                      whileHover={{ y: -5 }}
                      className="bg-white dark:bg-zinc-900 p-6 rounded-[2.5rem] border border-orange-100 dark:border-orange-900/30 flex flex-col items-center text-center space-y-4 shadow-sm hover:shadow-orange-900/5 dark:hover:shadow-orange-500/5 hover:border-orange-200 dark:hover:border-orange-800 transition-all relative group/member"
                    >
                       <div className="relative">
                         <img 
                           src={member.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${member.email}`} 
                           className="w-16 h-16 rounded-[1.25rem] border-4 border-white dark:border-zinc-800 shadow-xl" 
                           alt={member.displayName} 
                         />
                         {member.uid === family?.ownerId && (
                           <div className="absolute -top-3 -right-3 bg-orange-600 text-white p-1.5 rounded-xl border-2 border-white dark:border-zinc-800 shadow-lg">
                             <ShieldCheck className="w-3.5 h-3.5" />
                           </div>
                         )}
                         {member.familyStatus === 'suspended' && (
                            <div className="absolute -top-3 -left-3 bg-red-600 text-white p-1.5 rounded-xl border-2 border-white dark:border-zinc-800 shadow-lg animate-pulse">
        ...
        <div className="lg:col-span-4 space-y-8">
           <div className="bg-white dark:bg-zinc-900 p-10 rounded-[3rem] border border-black/5 dark:border-white/5 shadow-sm flex flex-col items-center text-center space-y-8">
              <div className="w-20 h-20 bg-green-50 dark:bg-green-500/10 text-green-600 dark:text-green-400 rounded-[2rem] flex items-center justify-center">
                 <ShieldCheck className="w-10 h-10" />
              </div>
              <div>
                <h4 className="text-xl font-bold tracking-tight italic serif text-black dark:text-white">Encrypted Enclave</h4>
                <p className="text-xs text-gray-400 dark:text-gray-500 font-light mt-4 leading-relaxed px-2">Your household records are strictly partitioned by standard AES-256 protocols. Only verified residents of <span className="text-black dark:text-white font-semibold uppercase tracking-tighter">{family?.name || 'Inzuka'}</span> can initiate read/write events on collective data points.</p>
              </div>
           </div>

           <div className="bg-white dark:bg-zinc-900 p-10 rounded-[3.5rem] border border-black/5 dark:border-white/5 shadow-sm space-y-10">
              <div className="flex items-center justify-between border-b border-gray-50 dark:border-zinc-800 pb-6">
                <h3 className="text-md font-bold flex items-center gap-3 italic serif text-black dark:text-white"><Users className="w-5 h-5 text-orange-500" /> Linked Nodes</h3>
                <button 
                  onClick={() => setShowAddRelation(!showAddRelation)}
                  disabled={!familyMembers.length || familyMembers.length < 2}
                  className="w-12 h-12 bg-black dark:bg-white text-white dark:text-black rounded-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-transform shadow-xl disabled:opacity-20"
                >
              ...
              <div className="space-y-5">
                 {profile?.relationships && profile.relationships.length > 0 ? (
                   profile.relationships.map((rel, idx) => {
                     ...
                     return (
                        <div key={idx} className="flex items-center justify-between p-5 rounded-[2rem] bg-gray-50 dark:bg-zinc-800 border border-black/[0.02] dark:border-white/[0.02] hover:bg-gray-100 dark:hover:bg-zinc-700 transition-colors">
                           <div className="flex items-center gap-4">
                              <img 
                                src={targetUser?.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${targetUser?.email}`} 
                                className="w-10 h-10 rounded-2xl bg-white dark:bg-zinc-800 shadow-sm" 
                                alt="Rel" 
                              />
                              <div>
                                 <p className="text-xs font-bold text-gray-900 dark:text-gray-100">{targetUser?.displayName || 'Legacy Record'}</p>
                                 <p className="text-[10px] text-orange-500 font-black uppercase tracking-widest mt-0.5">{rel.relationType}</p>
                              </div>
                           </div>
                           <button onClick={() => removeRelationship(rel.targetUserId)} className="p-2 text-gray-300 dark:text-gray-600 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-xl transition-all">
                   ...
                   <div className="py-12 text-center text-gray-300 dark:text-gray-600 italic text-sm font-light px-4 leading-relaxed">Establish direct links with residents to build your operational family tree.</div>
                 )}
              </div>
              ...
              <AnimatePresence>
                {showAddRelation && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                    <div className="space-y-4 p-8 bg-gray-100/50 dark:bg-zinc-800/50 rounded-[2.5rem] border border-black/5 dark:border-white/5 mt-4">
                       <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500 mb-2">Relational Node Setup</p>
                       <div className="space-y-3">
                         <select 
                           value={newRelationId}
                           onChange={e => setNewRelationId(e.target.value)}
                           className="w-full bg-white dark:bg-zinc-900 text-black dark:text-white rounded-2xl p-4 text-sm font-bold border border-black/5 dark:border-white/5 outline-none appearance-none cursor-pointer"
                         >
                       ...
                       <div className="flex gap-2 pt-2">
                          <button onClick={addRelationship} className="flex-1 bg-black dark:bg-white text-white dark:text-black py-4 rounded-xl font-bold text-[10px] uppercase tracking-[0.2em]">Link Entry</button>
                          <button onClick={() => setShowAddRelation(false)} className="flex-1 bg-white dark:bg-zinc-900 text-gray-400 dark:text-gray-500 py-4 rounded-xl font-bold text-[10px] uppercase tracking-[0.2em] border border-black/5 dark:border-white/5">Cancel</button>
                       </div>
                    </div>
                  </motion.div>
                )}
