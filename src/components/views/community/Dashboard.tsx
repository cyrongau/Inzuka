...
export default function CommunityDashboard({ 
  ...
  const stats = [
    { label: 'Active Networks', value: communities.length.toString(), icon: Globe2, color: 'text-blue-500', bg: 'bg-blue-50' },
          ...
          <div className="relative z-10 space-y-4">
             <div className="space-y-1">
                ...
                <p className="text-white/60 font-medium text-lg max-w-2xl">
                   Connect beyond your household. Manage fundraisers, table banking groups, and extended family networks seamlessly.
                </p>
             </div>
          </div>
        </div>
           ...
           <div className="bg-white rounded-[2.5rem] p-8 border border-black/5 shadow-sm">
             <div className="flex items-center justify-between mb-8">
               <h3 className="text-xl font-semibold flex items-center gap-2 italic serif">
                  <Users className="w-5 h-5" /> My Networks
               </h3>
               <button 
                 onClick={() => setShowWizard(true)}
                 className="flex items-center gap-2 px-4 py-2 bg-black text-white rounded-xl text-xs font-bold uppercase tracking-widest hover:scale-105 transition-transform"
               >
             ...
             {communities.length === 0 ? (
               <div className="text-center py-16 px-4 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                 ...
                 <h4 className="text-lg font-bold text-gray-900 mb-2">No Networks Yet</h4>
                 <p className="text-sm text-gray-500 max-w-md mx-auto">
                    You haven't joined or created any community groups. Set up a Table Banking group, a Fundraiser, or an Extended Family Hub to get started.
                 </p>
               </div>
             ) : (
               <div className="space-y-4">
                 {communities.map(comm => (
                   <div 
                     ...
                     className="p-5 bg-gray-50 border border-black/5 rounded-2xl flex items-center gap-4 hover:border-black/20 transition-colors cursor-pointer group"
                    >
                       <div className="w-12 h-12 bg-white rounded-xl border border-black/5 flex items-center justify-center shrink-0 text-xl shadow-sm">
                         {comm.type === 'Fundraising' ? '🤲' : comm.type === 'Table Banking' ? '🏦' : '👥'}
                       </div>
                       <div className="flex-1">
                         <h4 className="font-bold text-gray-900">{comm.name}</h4>
                         <p className="text-[10px] font-black uppercase text-blue-500 tracking-widest mt-0.5">{comm.type} • {comm.memberIds.length} Members</p>
                       </div>
                       <ArrowUpRight className="w-5 h-5 text-gray-300 group-hover:text-black transition-colors" />
                     </div>
                   ))}
        ...
        <div className="space-y-6">
           <div className="bg-gradient-to-br from-blue-500 to-purple-600 rounded-[2.5rem] p-8 text-white shadow-lg relative overflow-hidden group">
              ...
              <div className="relative z-10">
                 <h3 className="text-xl font-bold italic serif mb-4">Join an Existing Network</h3>
           ...
           <div className="bg-white rounded-[2.5rem] p-8 border border-black/5 shadow-sm">
              <h3 className="text-lg font-bold italic serif mb-4">Trending Scenarios</h3>
              <div className="space-y-3">
                 <div onClick={() => onOpenForum('funeral-planning-guide')} className="p-4 bg-orange-50 rounded-2xl cursor-pointer hover:bg-orange-100 transition-colors">
                    <p className="text-xs font-bold text-orange-900 leading-relaxed mb-1">Funeral Committee</p>
                    <p className="text-[10px] text-orange-600/70 uppercase tracking-widest font-black">Plan & Fundraise</p>
                 </div>
                 <div onClick={() => onOpenForum('chama-governance-101')} className="p-4 bg-green-50 rounded-2xl cursor-pointer hover:bg-green-100 transition-colors">
                    <p className="text-xs font-bold text-green-900 leading-relaxed mb-1">Chama / Table Banking</p>
                    <p className="text-[10px] text-green-600/70 uppercase tracking-widest font-black">Manage Group Savings</p>
                 </div>
                 <div onClick={() => onOpenForum('family-reunion-funding')} className="p-4 bg-blue-50 rounded-2xl cursor-pointer hover:bg-blue-100 transition-colors">
                    <p className="text-xs font-bold text-blue-900 leading-relaxed mb-1">Extended Family Reunion</p>
                    <p className="text-[10px] text-blue-600/70 uppercase tracking-widest font-black">Organize & Pool Funds</p>
                 </div>
              </div>
           </div>
        </div>
