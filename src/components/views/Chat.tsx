...
export default function Chat({ user, profile }: { user: AuthUser, profile: any }) {
  ...
  if (!profile?.familyId) {
    return (
        <div className="flex flex-col items-center justify-center p-20 text-center space-y-6 bg-white rounded-[3rem] border border-black/5">
            <div className="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center text-gray-400">
                <MessageCircle className="w-10 h-10" />
            </div>
            <div className="space-y-2">
                <h3 className="text-2xl font-bold italic serif">Chat is Locked</h3>
                <p className="text-gray-400 max-w-xs mx-auto">You need to join or create a family group to start chatting with your loved ones.</p>
            </div>
        </div>
    );
  }
  ...
  return (
    <div className="h-[calc(100vh-180px)] flex bg-white rounded-[3rem] border border-black/5 shadow-sm overflow-hidden">
      {/* Channels Sidebar */}
      <div className="w-80 bg-gray-50 border-r border-black/5 flex flex-col">
         <div className="p-8 pb-4">
            <h2 className="text-xl font-black italic serif">Messages</h2>
         </div>
         <div className="flex-1 overflow-y-auto px-4 pb-8 space-y-2">
            <button 
               ...
               className={cn(
                 ...
                 activeChannel === 'lounge' ? "bg-black text-white shadow-xl shadow-black/10" : "hover:bg-white text-gray-400 font-bold"
               )}
            >
            ...
            <div className="pt-6 pb-2 px-4">
              <span className="text-[10px] font-black uppercase tracking-widest text-gray-300">Direct Nodes</span>
            </div>
            ...
            {(Object.values(familyMembers) as any[]).filter(m => m.uid !== user.uid).map(member => (
              <button 
                ...
                className={cn(
                  "w-full flex items-center gap-3 p-4 rounded-2xl transition-all",
                  activeChannel === member.uid ? "bg-white text-black shadow-lg border border-black/5" : "hover:bg-white text-gray-400 font-bold"
                )}
              >
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="px-8 py-6 bg-white border-b border-gray-50 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-[2rem] bg-indigo-50 border border-black/5 flex items-center justify-center overflow-hidden shrink-0 shadow-sm relative">
              <img 
            ...
            <div>
              <h3 className="font-bold text-xl italic serif leading-tight">
          ...
          <div className="flex items-center gap-3">
              <button onClick={handleGroupCall} className="p-3 hover:bg-gray-50 rounded-2xl transition-colors"><PhoneCall className="w-5 h-5 text-gray-400" /></button>
              <button onClick={handleGroupCall} className="p-3 hover:bg-gray-50 rounded-2xl transition-colors"><VideoCam className="w-5 h-5 text-gray-400" /></button>
              <button className="p-3 hover:bg-gray-50 rounded-2xl transition-colors"><MoreVertical className="w-5 h-5 text-gray-400" /></button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-gray-50/30">
          <div className="text-center py-10">
              <span className="bg-white px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest text-gray-300 border border-black/[0.03]">Today</span>
          </div>
          ...
          {messages.map((msg, idx) => {
            ...
            return (
              ...
              >
                <div className="w-10 h-10 rounded-2xl bg-gray-100 overflow-hidden shrink-0 shadow-md border-2 border-white self-end mb-4">
                ...
                <div className={cn("flex flex-col gap-1 max-w-[75%]", isMe ? "items-end" : "items-start")}>
                   {!isMe && (
                     <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-2 mb-1">{senderName.split(' ')[0]}</span>
                   )}
                   <div className={cn(
                       "p-5 rounded-[2.5rem] shadow-sm relative transition-all group-hover:shadow-md",
                       isMe ? "bg-black text-white rounded-br-none" : "bg-white text-black rounded-bl-none border border-black/5"
                   )}>
                     {msg.mediaUrl && (
        ...
        <div className="p-8 bg-white border-t border-gray-50 space-y-4">
              ...
              {smartReply && !loadingReply && (
                  ...
                  >
                      ...
                      <button 
                          ...
                          className="bg-gray-100 hover:bg-gray-200 text-gray-600 px-4 py-2 rounded-2xl text-xs font-bold transition-all"
                      >
                          {smartReply}
                      </button>
                      <button onClick={() => setSmartReply('')} className="p-2 text-gray-300 hover:text-gray-900"><X className="w-3 h-3" /></button>
                  </motion.div>
              )}
          </AnimatePresence>
          ...
          <form onSubmit={handleSend} className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => sendMedia('image')} className="p-3 text-gray-400 hover:text-black hover:bg-gray-50 rounded-2xl transition-all"><ImageIcon className="w-5 h-5" /></button>
              <button type="button" onClick={() => sendMedia('video')} className="p-3 text-gray-400 hover:text-black hover:bg-gray-50 rounded-2xl transition-all"><Video className="w-5 h-5" /></button>
            </div>
            ...
            <div className="flex-1 relative">
              <input 
                ...
                className="w-full bg-gray-50 border border-black/5 rounded-[2rem] p-5 text-sm font-medium outline-none focus:bg-white focus:ring-1 focus:ring-black/10 transition-all"
              />
              <button type="button" className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-300 hover:text-black"><Smile className="w-5 h-5" /></button>
            </div>

            <button 
              type="submit"
              disabled={!inputText.trim()}
              className="w-16 h-16 bg-black text-white rounded-[2rem] flex items-center justify-center shadow-xl shadow-black/20 hover:scale-110 active:scale-95 transition-all disabled:opacity-20"
            >
          ...
          <div className="flex items-center justify-between px-2 pt-2">
              <div className="flex items-center gap-6">
                  <button className="flex items-center gap-2 text-[10px] font-black text-gray-300 uppercase tracking-widest hover:text-gray-900"><Mic className="w-4 h-4" /> Voice</button>
                  <button className="flex items-center gap-2 text-[10px] font-black text-gray-300 uppercase tracking-widest hover:text-gray-900"><Paperclip className="w-4 h-4" /> File</button>
              </div>
              <p className="text-[10px] font-black text-gray-200 uppercase tracking-widest">End-to-End Encrypted Node</p>
          </div>
        </div>
      </div>
