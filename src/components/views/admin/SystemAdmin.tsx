...
export default function SystemAdminDashboard({ user }: { user: User }) {
  ...
  return (
    <div className="p-6 lg:p-10 space-y-8 pb-24">
        ...
        {['hubs', 'reports', 'tickets', 'chats'].map((t) => (
          <button
            ...
            className={cn(
              ...
              activeTab === t ? "bg-white shadow-sm text-black" : "text-gray-500 hover:text-black"
            )}
          >
      ...
      {/* Control Panel Body */}
      <div className="bg-white rounded-[2.5rem] p-8 border border-black/5 shadow-sm space-y-8">
         {activeTab === 'hubs' && (
           <>
             <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div className="relative w-full md:w-96">
                   ...
                   <input 
                     ...
                     className="w-full bg-gray-50 border border-black/5 rounded-2xl pl-12 pr-4 py-3 outline-none focus:ring-2 focus:ring-black/10 transition-all font-medium text-sm"
                   />
                </div>
             </div>
                ...
                <table className="w-full text-left border-collapse min-w-[800px]">
                   <thead>
                     <tr className="border-b border-black/5">
                       <th className="pb-4 pt-2 font-bold text-[10px] uppercase tracking-widest text-gray-400 w-1/3">Network Identity</th>
                       <th className="pb-4 pt-2 font-bold text-[10px] uppercase tracking-widest text-gray-400">Architecture</th>
                       <th className="pb-4 pt-2 font-bold text-[10px] uppercase tracking-widest text-gray-400">Population</th>
                       <th className="pb-4 pt-2 font-bold text-[10px] uppercase tracking-widest text-gray-400">Status</th>
                       <th className="pb-4 pt-2 font-bold text-[10px] uppercase tracking-widest text-gray-400 text-right pr-4">Enforcement</th>
                     </tr>
                   </thead>
                   <tbody className="divide-y divide-black/[0.02]">
                     {filteredCommunities.map(comm => (
                       <tr key={comm.id} className="hover:bg-gray-50 transition-colors">
                          <td className="py-4">
                            <p className="font-bold text-sm tracking-tight">{comm.name}</p>
                            <p className="text-[10px] text-gray-500 font-mono mt-0.5">ID: {comm.id}</p>
                          </td>
                          <td className="py-4">
                            <span className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">{comm.type || 'Standard'}</span>
                          </td>
                          <td className="py-4">
                            <span className="font-bold text-sm">{comm.memberIds?.length || 0}</span> <span className="text-xs text-gray-400">Members</span>
                          </td>
                          <td className="py-4">
                            <div className={cn(
                               "inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest",
                               comm.status === 'active' ? "bg-green-50 text-green-600" :
                               comm.status === 'warned' ? "bg-orange-50 text-orange-600" :
                          ...
                          <td className="py-4 text-right flex justify-end gap-2 pr-4">
                            <button 
                              ...
                              className="w-8 h-8 rounded-xl flex items-center justify-center border border-black/5 text-gray-400 hover:text-green-500 hover:bg-green-50 transition-all font-bold"
                            > restore </button>
                          </td>
                       </tr>
                     ))}
              ...
              {reports.map(r => (
                <div key={r.id} className="bg-gray-50 p-6 rounded-3xl border border-black/5">
                   <div className="flex justify-between items-start mb-4">
                     <div>
                       <span className="bg-red-100 text-red-700 text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md">Reported</span>
                       <h4 className="font-bold text-lg mt-2">{r.subject}</h4>
                     </div>
                     <button 
                       onClick={() => handleStartChat(r.reporterId, r.reporterEmail, r.subject, r.id)}
                       className="px-4 py-2 bg-blue-50 text-blue-600 text-xs font-bold rounded-xl hover:bg-blue-100 flex items-center gap-1 transition-all"
                     >
                       <MessageSquare className="w-3 h-3" /> Chat with Reporter
                     </button>
                   </div>
                   <p className="text-sm text-gray-700 font-medium bg-white p-4 rounded-2xl border border-black/5">{r.reason}</p>
                </div>
              ))}
           </div>
         )}
           ...
           <div className="space-y-6">
              {tickets.map(t => (
                <div key={t.id} className="bg-gray-50 p-6 rounded-3xl border border-black/5">
                   ...
                   <p className="text-sm text-gray-700 font-medium bg-white p-4 rounded-2xl border border-black/5">{t.message}</p>
                </div>
              ))}
           </div>
         )}
         ...
         {activeTab === 'chats' && (
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 min-h-[500px]">
              <div className="space-y-3 lg:col-span-1 border-r border-black/5 pr-4 overflow-y-auto max-h-[500px]">
                 {conversations.map(c => (
                   <div 
                     key={c.id} 
                   ...
                   >
                      <p className="text-[8px] font-black uppercase tracking-widest mb-1 opacity-60">Target: {c.userEmail}</p>
                      <h4 className="font-bold text-sm truncate">{c.subject}</h4>
                   </div>
                 ))}
                 {conversations.length === 0 && <p className="text-center text-gray-400 py-12 text-xs italic">No active support channels.</p>}
              </div>

              <div className="lg:col-span-2 flex flex-col h-[500px] bg-gray-50 rounded-[2.5rem] border border-black/5 overflow-hidden">
                 {selectedChat ? (
                    <>
                      <div className="p-6 border-b border-black/5 bg-white flex items-center justify-between">
                         <div>
                            <h3 className="font-bold text-sm">{selectedChat.subject}</h3>
                            <p className="text-[10px] text-gray-400 font-medium">Recipient: {selectedChat.userEmail}</p>
                         </div>
                         <div className="px-3 py-1 bg-red-100 text-red-700 text-[8px] font-black uppercase tracking-widest rounded-full">Official Channel</div>
                      </div>
                               ...
                               <div className="flex flex-col gap-1 max-w-[80%]">
                                  {m.isAdmin && (
                                    <div className="flex items-center gap-2 justify-end mb-1">
                                       <span className="text-[10px] font-black uppercase text-red-600 tracking-widest">System Admin</span>
                                       <div className="w-6 h-6 rounded-full bg-red-600 flex items-center justify-center text-[10px] font-black text-white">OA</div>
                                    </div>
                                  )}
                                  <div className={cn(
                                    "p-4 rounded-[1.8rem] text-sm font-medium shadow-sm transition-all",
                                    m.isAdmin ? "bg-red-50 text-red-900 border border-red-100 rounded-tr-none" : "bg-white text-black border border-black/5 rounded-tl-none"
                                  )}>
                      ...
                      <form onSubmit={handleSendReply} className="p-4 bg-white border-t border-black/5 flex gap-2">
                         <input 
                           type="text" 
                           placeholder="Type administrative response..." 
                           value={replyText}
                           onChange={e => setReplyText(e.target.value)}
                           className="flex-1 bg-gray-50 border border-black/5 rounded-xl px-4 py-3 text-sm font-medium outline-none"
                         />
                         <button className="w-12 h-12 bg-black text-white rounded-xl flex items-center justify-center hover:scale-105 active:scale-95 transition-all">
                            <Send className="w-5 h-5" />
                         </button>
                      </form>
                    </>
                 ) : (
                    <div className="h-full flex items-center justify-center text-gray-400 font-medium italic text-sm">Select a channel.</div>
                 )}
              </div>
           </div>
         )}
