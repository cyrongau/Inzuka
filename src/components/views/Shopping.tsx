      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <>
            <div className="lg:col-span-4 space-y-6">
              <div className="flex items-center justify-between px-2">
                <h3 className="text-xl font-bold tracking-tight">Shopping Groups</h3>
                <button onClick={() => setShowAddList(!showAddList)} className="w-10 h-10 bg-black text-white rounded-xl shadow-lg flex items-center justify-center hover:scale-110 active:scale-95 transition-transform"><Plus className="w-5 h-5" /></button>
              </div>

              {showAddList && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-6 rounded-[2rem] border border-black/5 shadow-xl space-y-4">
                  <input placeholder="List Name" value={newListName} onChange={e => setNewListName(e.target.value)} className="w-full bg-gray-50 rounded-xl p-4 border border-black/5 font-medium" />
                  <div className="flex gap-2">
                    <button onClick={createList} className="flex-1 bg-black text-white py-4 rounded-xl font-bold">Create</button>
                    <button onClick={() => setShowAddList(false)} className="px-6 py-4 bg-gray-100 rounded-xl font-bold text-gray-500">Cancel</button>
                  </div>
                </motion.div>
              )}
              ...
              <div className="space-y-3">
                {lists.map(list => (
                  <div key={list.id} onClick={() => setSelectedListId(list.id)} className={cn("p-5 rounded-[2rem] cursor-pointer border transition-all flex items-center justify-between group", selectedListId === list.id ? "bg-black text-white border-black shadow-xl" : "bg-white border-black/5 hover:bg-black/[0.02]")}>
                    <div className="flex items-center gap-4">
                       <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center transition-colors", selectedListId === list.id ? "bg-white/20 text-white" : "bg-gray-100 text-gray-400 group-hover:bg-gray-200")}>
            ...
            <div className="lg:col-span-8 bg-white rounded-[3rem] p-10 border border-black/5 shadow-sm min-h-[600px] flex flex-col">
              {selectedList ? (
                <div className="space-y-8 flex-1 flex flex-col">
                  <div className="flex items-start justify-between border-b border-black/5 pb-8">
                    <div className="space-y-4">
                      <div>
                        <h2 className="text-4xl font-bold tracking-tighter serif italic">{selectedList.name}</h2>
                        <div className="flex items-center gap-3 mt-1">
                          ...
                          <p className="text-xs text-gray-400 font-bold uppercase tracking-widest">{selectedList.items?.length || 0} Items Total</p>
                        </div>
                      </div>
                      ...
                      {selectedList.items?.length > 0 && (
                        <div className="flex flex-col gap-1">
                          <div className={cn(
                            ...
                            calculateListEstimate(selectedList.items).confidence > 50 ? "bg-green-50 border-green-100 text-green-700" : "bg-gray-50 border-black/5 text-gray-500"
                          )}>
                  ...
                  <div className="flex gap-3">
                    <input 
                       ...
                       className="flex-1 bg-gray-50 border border-black/5 rounded-2xl px-6 py-4 text-lg font-medium focus:ring-1 focus:ring-black/10 focus:outline-none"
                    />
                    <button onClick={addItem} className="bg-black text-white px-10 rounded-2xl font-bold hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg">Add Item</button>
                  </div>

                  <div className="space-y-3 flex-1">
                    {selectedList.items?.sort((a:any, b:any) => Number(a.bought) - Number(b.bought)).map((item: any) => (
                      <div key={item.id} className={cn("flex items-center justify-between p-5 rounded-3xl border transition-all group", item.bought ? "bg-gray-50 border-transparent opacity-60" : "bg-white border-black/[0.05] hover:border-black/20 hover:shadow-md")}>
                        <div className="flex items-center gap-4">
                           <button 
                             onClick={() => {
                             ...
                             className={cn("w-10 h-10 rounded-xl flex items-center justify-center border transition-all hover:scale-105 active:scale-95", item.bought ? "bg-black border-black text-white shadow-lg" : "border-black/10 text-gray-200 bg-white hover:bg-gray-50")}
                           >
                             <CheckCircle2 className="w-5 h-5" />
                           </button>
                           <span className={cn("text-lg font-bold", item.bought && "line-through text-gray-400 italic font-medium")}>{item.name}</span>
                        </div>
                        <button 
                          onClick={() => {
        ...
        {activeTab === 'scanner' && (
          <div className="lg:col-span-12 max-w-4xl mx-auto w-full space-y-8">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-black text-white p-10 rounded-[4rem] shadow-2xl relative overflow-hidden flex flex-col justify-between">
                   <div className="relative z-10 space-y-8">
                      <div className="flex items-center justify-between">
                         <div className="space-y-1">
                           <h3 className="text-3xl font-bold tracking-tight italic serif">Barcode Lens</h3>
                           <p className="text-xs font-bold text-white/40 uppercase tracking-widest">Standard UPC/EAN</p>
                         </div>
                      ...
                      {!scannedCode ? (
                        <div className="space-y-6">
                           <div id="reader" className={cn("bg-white/5 rounded-[2.5rem] overflow-hidden min-h-[300px] border-2 border-dashed border-white/20 transition-all flex items-center justify-center relative", !isScanning && "hidden")}>
                             <div className="absolute inset-0 border-4 border-white/10 m-8 rounded-3xl animate-pulse"></div>
                           </div>
                           {!isScanning && (
                              <button 
                                onClick={() => setIsScanning(true)}
                                className="w-full bg-white text-black py-10 rounded-[2.5rem] font-bold text-xl flex flex-col items-center gap-4 hover:bg-gray-100 transition-all shadow-[0_20px_50px_rgba(255,255,255,0.1)] group"
                              >
                                 <div className="w-14 h-14 bg-black text-white rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform"><Camera className="w-7 h-7" /></div>
                                 Start Camera Scan
                              </button>
                           )}
                           {isScanning && (
                             <button onClick={() => setIsScanning(false)} className="w-full py-4 bg-white/10 rounded-2xl font-bold hover:bg-white/20">Cancel Camera</button>
                           )}
                        </div>
                      ) : (
                        <p className="text-white/60 italic font-medium">Scanned successfully. View results below.</p>
                      )}
                   </div>
                   
                   <p className="text-[10px] items-center gap-2 flex text-white/30 uppercase tracking-[0.2em] font-bold mt-8">
             ...
             <div className="relative">
                {scannedCode && (
                   <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white text-black p-10 rounded-[3rem] space-y-8 shadow-2xl relative z-10">
                      {/* ... existing barcode result content ... */}
                      <div className="flex items-center gap-6">
                         <div className="w-20 h-20 bg-black rounded-[2rem] flex items-center justify-center text-white shadow-xl rotate-3"><Package className="w-10 h-10" /></div>
                         <div>
                            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Product Detected</p>
                            <h4 className="text-3xl font-bold tracking-tighter">{matchingProduct?.name}</h4>
                            <div className="flex items-center gap-2 mt-1">
                              <Scan className="w-3 h-3 text-gray-300" />
                              <span className="text-[10px] font-mono text-gray-400 tracking-wider">CODE: {scannedCode}</span>
                            </div>
                         </div>
                      </div>
                      ...
                      <div className="space-y-6 pt-6 border-t border-black/5">
                         ...
                         <div className="space-y-4 pt-4">
                           <p className="text-xs font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2"><Scale className="w-4 h-4" /> Current Price (KES)</p>
                           <div className="relative">
                             <input 
                               type="number" placeholder="0.00" value={newPrice} onChange={e => setNewPrice(e.target.value)}
                               className="w-full bg-gray-50 border border-black/5 rounded-2xl p-8 text-5xl font-black focus:outline-none focus:ring-4 focus:ring-black/5"
                             />
                             <div className="absolute right-6 top-1/2 -translate-y-1/2 text-gray-300 font-bold uppercase tracking-widest text-xs">KES</div>
                           </div>
                         </div>
                         
                         {matchingProduct?.baselinePrice && (
                           <div className={cn("p-6 rounded-2xl flex items-center justify-between text-sm font-bold shadow-inner", parseFloat(newPrice) > matchingProduct.baselinePrice ? "bg-red-50 text-red-600" : "bg-green-50 text-green-600")}>
                         ...
                         <div className="flex gap-3 pt-4">
                           <button onClick={recordPrice} className="flex-[2] bg-black text-white py-6 rounded-[2.5rem] font-black uppercase tracking-widest shadow-2xl hover:scale-105 active:scale-95 transition-transform">Record Price</button>
                           <button onClick={() => {setScannedCode(null); setMatchingProduct(null);}} className="flex-1 bg-gray-100 text-gray-500 rounded-[2.5rem] font-bold flex items-center justify-center p-6"><X className="w-8 h-8" /></button>
                         </div>
                      </div>
                   </motion.div>
                )}
                {/* Visual Flair Background */}
                <div className="absolute top-[-20%] left-[-10%] w-96 h-96 bg-black/[0.02] rounded-full blur-[80px]"></div>
                <div className="absolute bottom-[-20%] right-[-10%] w-64 h-64 bg-black/[0.02] rounded-full blur-[60px]"></div>
             </div>
          </div>
        )}
          ...
          <div className="lg:col-span-12 space-y-8 animate-in fade-in slide-in-from-bottom-5">
             <div className="bg-white p-12 rounded-[4rem] border border-black/5 shadow-sm">
                <div className="flex items-center justify-between mb-12">
                   <div className="space-y-1">
                     <h3 className="text-4xl font-black tracking-tight serif italic tracking-tighter">Pantry Price Index</h3>
                     <p className="text-xs uppercase tracking-widest font-bold text-gray-400">Personal Inflation Analytics</p>
                   </div>
                   <div className="flex items-center gap-4">
                      <button 
                        onClick={() => {
                        ...
                        className="bg-black text-white px-6 py-3 rounded-2xl flex items-center gap-3 text-xs font-bold hover:scale-110 active:scale-95 transition-transform shadow-xl"
                      >
                         <Plus className="w-4 h-4" /> Add Standard
                      </button>
                      <div className="bg-gray-100 text-gray-500 px-6 py-3 rounded-2xl flex items-center gap-3 text-xs font-bold border border-black/5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                   {products.map(product => {
                      ...
                      return (
                        <div key={product.id} onClick={() => setDetailProduct(product)} className="bg-gray-50/50 p-10 rounded-[3rem] border border-black/5 space-y-8 hover:bg-white hover:shadow-2xl transition-all group cursor-pointer relative overflow-hidden">
                           <div className="flex justify-between items-start relative z-10">
                              <div className="w-16 h-16 bg-white rounded-[1.5rem] shadow-sm border border-black/5 flex items-center justify-center group-hover:bg-black group-hover:text-white transition-all"><Package className="w-8 h-8" /></div>
        ...
        {activeTab === 'pantry' && (
          <div className="lg:col-span-12 space-y-8 animate-in fade-in zoom-in-95 duration-300">
             <div className="bg-white p-10 rounded-[3rem] border border-black/5 shadow-sm space-y-8 min-h-[500px]">
                <div className="flex items-center justify-between">
                   <div className="space-y-1">
                      <h3 className="text-3xl font-black italic serif tracking-tight">Household Stock</h3>
                      <p className="text-xs font-bold uppercase tracking-widest text-gray-400">Track pantry & cleaning supplies usage</p>
                   </div>
                   <button onClick={() => setShowPantryForm(true)} className="bg-black text-white px-6 py-4 rounded-2xl text-xs font-bold uppercase tracking-widest hover:scale-105 transition-all">
                      + Add Stock Item
                   </button>
                </div>
                     ...
                     {pantryItems.map(item => {
                        ...
                        return (
                          <div key={item.id} className={cn(
                            ...
                            isLow ? "border-red-500/30 bg-red-50/50" : "border-black/[0.05] bg-gray-50/50"
                          )}>
                             ...
                             <div className="flex items-center justify-between mb-4">
                               ...
                               <button onClick={() => deleteDoc(doc(db, 'inventory', item.id))} className="opacity-0 group-hover:opacity-100 p-2 text-red-400 hover:text-red-500 transition-all rounded-full hover:bg-red-50">
                                 <Trash2 className="w-3.5 h-3.5" />
                               </button>
                             </div>

                             <h4 className="text-xl font-black italic serif mb-1">{item.name}</h4>
                             <p className={cn("text-2xl font-black mb-6 flex items-baseline gap-1", isLow ? "text-red-500" : "text-black")}>
                             ...
                             <div className="space-y-4">
                               <div className="h-2 w-full bg-black/5 rounded-full overflow-hidden">
                                 <div 
                                    className={cn("h-full rounded-full transition-all", isLow ? "bg-red-500" : "bg-black")} 
                                    style={{ width: `${Math.min(usageRatio, 100)}%` }} 
                                 />
                               </div>
                               <div className="flex items-center gap-2">
                                  <button onClick={() => handleConsume(item)} className="flex-1 py-3 bg-white border border-black/5 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-gray-50">- Consume</button>
                                  <button onClick={() => updateDoc(doc(db, 'inventory', item.id), { quantity: item.quantity + 1 })} className="flex-1 py-3 bg-black text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-gray-900">+ Restock</button>
                               </div>
                             </div>
                          </div>
                        );
        ...
        {activeTab === 'inflation' && detailProduct && (
          <div className="lg:col-span-12 space-y-8 animate-in fade-in zoom-in-95 duration-300">
            <div className="bg-white rounded-[4rem] border border-black/5 shadow-2xl overflow-hidden min-h-[700px] flex flex-col">
              {/* Header */}
              <div className="p-12 bg-black text-white flex flex-col md:flex-row md:items-end justify-between gap-8 relative">
                ...
                <div className="lg:col-span-8 space-y-8">
                  <div className="flex items-center justify-between">
                    <h4 className="text-xl font-black tracking-tight flex items-center gap-3">
                      <TrendingUp className="w-6 h-6 text-gray-300" /> Price History (Time Series)
                    </h4>
                    <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-xl">
                      <button className="bg-white px-4 py-2 rounded-lg text-[10px] font-bold shadow-sm uppercase tracking-widest">Growth View</button>
                      <button className="text-gray-400 px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest hover:text-black">Comparative</button>
                    </div>
                  </div>
                  
                  <div className="h-[400px] w-full bg-gray-50/50 rounded-[3rem] p-8 border border-black/[0.02]">
